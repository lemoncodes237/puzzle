...
execute unless entity @a[tag=in_sublevel] unless entity @a[tag=creating] unless entity @a[tag=sublevel_ended] as @a at @s if entity @e[type=item_display,tag=puz_main,distance=..0.5] run function puzzle:sublevel/begin
execute if entity @a[tag=in_sublevel,tag=!sublevel_started] as @a at @s if entity @e[type=item_display,tag=puz_main,distance=..0.5,tag=puz_curr] run function puzzle:sublevel/end
execute if entity @a[tag=in_sublevel,tag=!sublevel_started] as @a at @s if entity @e[type=item_display,tag=puz_main,distance=..0.5,tag=!puz_curr] run title @s actionbar "You cannot enter a new state before exiting your current one" 

execute as @a[tag=sublevel_started] at @s unless entity @e[type=item_display,tag=puz_main,distance=..0.8] run tag @a remove sublevel_started
execute as @a[tag=sublevel_ended] at @s unless entity @e[type=item_display,tag=puz_main,distance=..0.8] run tag @a remove sublevel_ended
